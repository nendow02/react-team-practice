...
export default function Daniel(): JSX.Element {
  ...
  const handleChange = (didChange: boolean) => {
      if (didChange)
        console.log("TeachLA Member");
      else
        console.log("Not a TeachLA Member")
  }
  return (
    <div id="daniel">
      <input placeholder="Type HIDE to hide the button" 
      onChange={(e) => {
        setMyValue(e.target.value);
      }} />   
       {myValue !== 'HIDE' ?  
       <CustomButton text={number.toString()} 
       onClick={() => setNumber(number + 1)}/> : null}
        ...
        <div id="myCheckbox">
          ...
          I am a TeachLA Member</div>

      <input placeholder="What number to display?"
      onChange={(e) => {
